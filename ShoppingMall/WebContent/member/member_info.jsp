<%@ page language="java" contentType="text/html; charset=EUC-KR"
 ...
 <%
 	...
 	String[] zipcodes = MEMBER_ZIPCODE.split("-"); // "-"로 구분
 	String[] email = MEMBER_EMAIL.split("@"); // "@"로 구분
 %>
...
function check(form){
	...
	var email2 = inoform.MEMBER_EMAIL2.value;
	...
	if((forms.MEMBER_NAME.value=="") || (fomrs.MEMBER_NAME.value.length <= 1)){
		alert("이름을 입력하세요");
	...
	if(pass1.length == 0){
		alert("비밀번호를 입력하세요");
		forms.MEMBER_PW.focus();
		return false;		
	}
	if(pass1 != pass2){
		alert("비밀번호가 일치하지 않습니다.");
		forms.MEMBER_PW.value="";
		forms.MEMBER_PW.value="";
...
function out(){
	var Answer = confirm("회원 탈퇴를 하시겠습니까?");
	if(Answer == true){
...
<!-- 개인정보수정 -->
<p align="center">
<form action="./MemberModifyAction_2.me" method="post" name="infoform" onsubmit="return check(this)">
...
<table border="1" width="80%" height="80%">
	<tr>
		...
		<td>
			&nbsp;&nbsp;&nbsp;
			<input type="text" name="MEMBER_NAME" size="20" value="${member.MEMBER_NAME}"/>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
		<font size="2">&nbsp;&nbsp;&nbsp;&nbsp;
		(아이디와 비밀번호는 문자와 숫자를 조합하여 2~12자리로 만들어주세요)</font>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
		&nbsp;&nbsp;&nbsp;
		<input type="text" name="MEMBER_EMAIL1" size="13" value="<%=email[0].trim() %>"/>@
		<input type="text" name="MEMBER_EMAIL2" size="13" value="13" value="<%=email[1].trim()%>"/>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
		&nbsp;&nbsp;&nbsp;
		<input type="radio" name="MEMBER_EMAIL_GET" value="YES" <%if(MEMBER_EMAIL_GET.equals("YES")){%>checked<%} %>/>
		<font size="2">수신</font>
		&nbsp;&nbsp;
		<input type="radio" name="MEMBER_EMAIL_GET" value="NO" <%if(MEMBER_EMAIL_GET.equals("NO")){%>checked<%}%>/>
	...
	<tr>
		...
		<td>
			&nbsp;&nbsp;&nbsp;
			<input type="text" name="MEMBER_PHONE" size="24" value="${member.MEMBER_PHONE}" size="24" value="${member.MEMBER_PHONE}"/>		
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			&nbsp;&nbsp;&nbsp;
			<input type="text" name="MEMBER_ZIPCODE1" size="6" onkeypress="gNumCheck()" maxlength="3" value="<%=zipcodes[0].trim()%>"/>-
			<input type="text" name="MEMBER_ZIPCODE2" size="6" onkeypress="gNumCheck()" maxlength="3" value="<%=zipcodes[1].trim()%>"/>&nbsp;&nbsp;
	...
	<tr>
		...
		<td>
			&nbsp;&nbsp;&nbsp;
			<input type="text" name="MEMBER_ADDR1" size="50" value="${member.MEMBER_ADDR1}"/>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			&nbsp;&nbsp;&nbsp;
			<input type="text" name="MEMBER_ADDR2"  size="50" value="${member.MEMBER_ADDR2}"/>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			&nbsp;&nbsp;&nbsp;
			<input type="text" name="MEMBER_MOBILE" size="24" value="${member.MEMBER_MOBILE}"/>
		</td>
	</tr>				
</table>
...
<table width="80%">
	<tr>
		<td align="center">
			...
			<input type="button" value="회원탈퇴" name="bt" onclick="out()"/>
		</td>
	</tr>
</table>
</form>
